...
(
	input clk,
	input reset,
	// 2 agosto 2026: era [15:0]. Con una cartuccia da 128 KB l'indirizzo di
	// download tornava a zero a meta' file, e sia `match_bytes` sia il blocco
	// CRC di `has_sc` usano `addr==0` come "inizio immagine": tutti i
	// riconoscimenti trovati nei primi 64 KB venivano AZZERATI. Il tag
	// "PLUSCDFJ" di Turbo Arcade sta all'offset 388 e non ricompare piu': per
	// questo Turbo non veniva mai riconosciuto come CDFJ+.
	input [24:0] addr,
	input enable,
	...
	// 5 agosto 2026 - riconoscimento automatico del QUADTARI, come fa Stella
	// (ControllerDetector::isProbablyQuadTari, ControllerDetector.cxx:586).
	// Serve perche' i titoli Champ Games alternano i due controller di ogni
	// porta scrivendo il bit 7 di VBLANK: senza QuadTari la porta risponde due
	// volte con lo STESSO pad, quindi premendo il fuoco il gioco vede premuto
	// anche il pulsante del secondo giocatore e si comporta come se avessi
	// premuto Select. Succede solo su Elevator Agent e Tutankham, ed e' quello
	// che il PM ha osservato (identico anche con lo SNAC, perche' il difetto
	// non e' nella mappatura ma nella selezione).
	output quadtari
);

wire hasMatch3F;

// (decision block moved to end of module so every hasMatch* wire is declared
// before use - required by Icarus Verilog, no functional change)
// (bs decision block moved to end of module - see below)
...
// CDF family sub-version detection, mirrors Stella's CartDetector: the
// cartridge header repeats an ASCII tag ("CDF", "CDFJ" or "PLUSCDFJ")
// whose exact form identifies CDF0/1 vs CDFJ vs CDFJ+. cdf_family feeds
// atari2600_arm_subsystem's family_sel so each variant gets its own
// Harmony RAM layout / callback addresses instead of one hardcoded guess.
wire  hasMatchCDF_1,hasMatchCDF_2,hasMatchCDF_J;
wire hasMatchCDF =  hasMatchCDF_1  | hasMatchCDF_2 | hasMatchCDF_J;

//----------------------------------------------------------------------------
// QUADTARI. Stella cerca tre firme; qui bastano le due che compaiono davvero
...
// fa scorrere il nuovo byte nei bit bassi).
//----------------------------------------------------------------------------
...
match_bytes #(
	.num_bytes(8'd8),
	.pattern({ 8'h51, 8'h55, 8'h41, 8'h44, 8'h54, 8'h41, 8'h52, 8'h49 }), // "QUADTARI"
	.needmatches(8'd1)
...
// "CDFJ" without the "PLUS" prefix identifies plain CDFJ (not CDFJ+).
match_bytes #(
	.num_bytes(8'd4),
...
// 2'b00=CDF0/1, 2'b01=CDFJ, 2'b10=CDFJ+. hasMatchCDF_* are sticky for the
// whole download, so the priority must be evaluated once at the end
// (combinationally from the latched flags), not as a per-cycle race that
// depends on which tag happens to appear later in the file.
...
// Turbo Arcade (CDFJ+, 131072 byte) non partiva: veniva riconosciuto come
// BANK3F. `hasMatch3F` cerca due occorrenze di `85 3F` (STA $3F) e in un file
// da 128 KB quella coppia di byte compare per caso; il test 3F stava PRIMA del
// test CDF, quindi vinceva lui. Stella prova `isProbablyCDF` prima di
// 3EX/3E/3F/SB per ogni taglia da 32 KB in su (CartDetector.cxx:124, 143, 154,
// 164, 174): la nostra catena aveva l'ordine invertito.
// Stella mette CTY (32 KB) ed EFF (64 KB) sopra CDF. Qui il CDF sta sopra a
// tutto: verificato per misura sull'intera collezione (21 CDF + 5 CDFJ+ + 25
// DPC+ + Chetiry/CTY) che NESSUNA ROM combacia con due famiglie insieme -
// l'unica sovrapposizione esistente e' proprio Turbo con 3F.
...
(
	input clk,
	input reset,
	input  [24:0] addr,    // 2 ago 2026: allargato, vedi detect2600
	input  enable,
