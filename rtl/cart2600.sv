...
(
	...
	assign rom_read = (mapper == BANKAR) ? ar_read :
	                  ((mapper == BANKCDF || mapper == BANKDPCP) && arm_enable) ? arm_rom_read :
	                  ~address_change;
	wire is_bad_game = ~arm_enable & (mapper == BANKDPCP || mapper == BANKCDF);
	...
	atari2600_arm_subsystem u_arm_subsystem (
		...
		.d_in          (d_in),
		.phi1          (phi1),
		.we            (~phi1 & ~address_change & sel_ram_sel & ~sel_ram_rw),
	...
	assign direct_do[BANKDPCP]    = arm_enable ? arm_dout : bg_data;
	...
	assign rom_addr[BANKDPCP]     = arm_enable ? arm_rom_a : '0;
