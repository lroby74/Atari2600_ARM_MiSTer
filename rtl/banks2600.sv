...
(
	...
	// Cyclone V (DE10-nano) M10K block RAM; Supercharger tape RAM (dual-clock).
	// R-D-W declared don't-care via no_rw_check (write during load, read during play).
	(* ramstyle = "no_rw_check" *) logic [7:0] block_byte_array [0:23];
	logic [7:0] block_checksum;
	// Cyclone V M10K block RAM; Supercharger checksum RAM (dual-clock), R-D-W don't-care via no_rw_check.
