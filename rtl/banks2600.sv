...
(
	...
	always @(posedge clk) begin
		...
		if (reset)
			for (int ei = 0; ei < 3; ei++) banks[ei] <= 3'd0;
	end
...
(
	...
	// 4 ago 2026: stessa riscrittura fatta in video_mux.sv - array unpacked con
	// '{...} rifiutata da Verilator. Ordine INVERTITO (indice 0 = bit bassi):
	//   idx:  7   6   5   4   3   2   1   0
	//   val: 00  04  05  09  06  0A  0B  0F
	wire [63:0] amplitude_lut = {8'h00, 8'h04, 8'h05, 8'h09, 8'h06, 8'h0A, 8'h0B, 8'h0F};
	wire [7:0] rom_do_i = ~|dpc_addr ? (~music_en ? rand_val : amplitude_lut[music_amp_index*8 +: 8]) :
...
(
	...
	// Supercharger tape byte storage (2×24 bytes, R-D-W don't-care).
	(* ramstyle = "no_rw_check" *) logic [7:0] block_byte_array [0:23];
	logic [7:0] block_checksum;
	(* ramstyle = "no_rw_check" *) logic [7:0] cs_array [0:23];

	// Banks 0-2 are the ram chip selects, bank 3 is the boot rom
	assign bank_lut[0][0] = 2'd2;
	assign bank_lut[0][1] = 2'd3;
	assign bank_lut[1][0] = 2'd0;
	assign bank_lut[1][1] = 2'd3;
	assign bank_lut[2][0] = 2'd2;
	assign bank_lut[2][1] = 2'd0;
	assign bank_lut[3][0] = 2'd0;
	assign bank_lut[3][1] = 2'd2;
	assign bank_lut[4][0] = 2'd2;
	assign bank_lut[4][1] = 2'd3;
	assign bank_lut[5][0] = 2'd1;
	assign bank_lut[5][1] = 2'd3;
	assign bank_lut[6][0] = 2'd2;
	assign bank_lut[6][1] = 2'd1;
	assign bank_lut[7][0] = 2'd1;
	assign bank_lut[7][1] = 2'd2;
	...
	assign ar_read = ce;
	assign d_out = ~ram_rw ? we_byte : (adata_select ? {7'd0, audio_data} : sc_rom_data);
			...
			case (state)
				...
				AR_LOAD_HEADER: begin
					...
					if (header_a < 7) begin
						...
						header_a <= 0;
						header_array[4] <= 8'h55 - (header_array[0] + header_array[1] + header_array[2] + header_array[3] + header_array[4] + header_array[5] + header_array[6] + header_array[7]);
				...
				AR_CALC_CHECKSUMS: begin
					if (&bank_a) begin
						...
						if ((page_position == (page_count - 1'd1)) || (page_position == 23)) state_next <= AR_PREAMBLE; else state_next <= AR_LOAD_BLOCK_BYTE;				
					end else begin
				...
				AR_PREAMBLE: begin
					...
					state_count <= state_count + 1'd1;
					if (&state_count[8:0]) state_next <= AR_HEADER; else state_next <= AR_PREAMBLE;
					state <= AR_FETCH;
				end
				AR_HEADER: begin
					if (fix_sc_cs)
				...
				AR_BANK: begin
					...
					if (&bank_a) begin
						page_position <= page_position + 1'd1;
						if ((page_position == (page_count - 1'd1)) || (page_position == 23)) state_next <= AR_POSTAMBLE; else state_next <= AR_BLOCK_BYTE;
					end else
						state_next <= AR_BANK;
					bank_a <= bank_a + 1'd1;
					state <= AR_FETCH;
				...
				AR_POSTAMBLE: begin
					...
					state_count <= state_count + 1'd1;
					state <= AR_FETCH;
					if (&state_count[8:0]) state_next <= AR_END; else state_next <= AR_POSTAMBLE;
...
(
	...
	assign bank_lut[0][0] = 3'd0;
	assign bank_lut[0][1] = 3'd0;
	assign bank_lut[0][2] = 3'd1;
	assign bank_lut[0][3] = 3'd3;
	assign bank_lut[1][0] = 3'd0;
	assign bank_lut[1][1] = 3'd1;
	assign bank_lut[1][2] = 3'd2;
	assign bank_lut[1][3] = 3'd3;
	assign bank_lut[2][0] = 3'd4;
	assign bank_lut[2][1] = 3'd5;
	assign bank_lut[2][2] = 3'd6;
	assign bank_lut[2][3] = 3'd7;
	assign bank_lut[3][0] = 3'd7;
	assign bank_lut[3][1] = 3'd4;
	assign bank_lut[3][2] = 3'd2;
	assign bank_lut[3][3] = 3'd3;
	assign bank_lut[4][0] = 3'd0;
	assign bank_lut[4][1] = 3'd0;
	assign bank_lut[4][2] = 3'd6;
	assign bank_lut[4][3] = 3'd7;
	assign bank_lut[5][0] = 3'd0;
	assign bank_lut[5][1] = 3'd1;
	assign bank_lut[5][2] = 3'd7;
	assign bank_lut[5][3] = 3'd6;
	assign bank_lut[6][0] = 3'd2;
	assign bank_lut[6][1] = 3'd3;
	assign bank_lut[6][2] = 3'd4;
	assign bank_lut[6][3] = 3'd5;
	assign bank_lut[7][0] = 3'd6;
	assign bank_lut[7][1] = 3'd0;
	assign bank_lut[7][2] = 3'd5;
	assign bank_lut[7][3] = 3'd1;
	...
	always @(posedge clk) begin
		if (a_change) begin
			...
			if (pending_switch[3]) begin
				for (int li = 0; li < 4; li++) banks[li] <= bank_lut[bank_config][li];
			end
		...
		if (reset) begin
			bank_config <= 0;
			pending_switch <= 0;
			for (int li = 0; li < 4; li++) banks[li] <= bank_lut[0][li];
		end
	end
endmodule
